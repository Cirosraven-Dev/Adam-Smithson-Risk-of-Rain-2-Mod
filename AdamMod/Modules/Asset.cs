...
namespace AdamMod.Modules
{
    internal static class Asset
    {
        ...
        internal static AssetBundle LoadAssetBundle(string bundleName)
        {
            ...
            try
            {
                assetBundle = AssetBundle.LoadFromFile(Path.Combine(Path.GetDirectoryName(AdamPlugin.instance.Info.Location), bundleName));
            }
        ...
        internal static GameObject CloneTracer(string originalTracerName, string newTracerName)
        {
            ...
            var newTracer = LegacyResourcesAPI.Load<GameObject>("Prefabs/Effects/Tracers/" + originalTracerName).InstantiateClone(newTracerName, true);
        ...
        {
            var loadedCrosshair = LegacyResourcesAPI.Load<GameObject>("Prefabs/Crosshair/" + crosshairName + "Crosshair");
        ...
        internal static GameObject LoadEffect(this AssetBundle assetBundle, string resourceName, bool parentToTransform) => assetBundle.LoadEffect(resourceName, "", parentToTransform);
        ...
        {
            var newPrefab = LegacyResourcesAPI.Load<GameObject>("Prefabs/Projectiles/" + prefabName).InstantiateClone(newPrefabName);
            return newPrefab;
        }
